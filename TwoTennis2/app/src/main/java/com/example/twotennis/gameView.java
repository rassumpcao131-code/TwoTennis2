...
public class gameView extends SurfaceView
    ...
    private static final float MAX_PIXELS_MODO_RAPIDO = 1_050_000f;
    private static final float LARGURA_MAXIMA_MODO_RAPIDO = 720f;
    ...
    private final ArrayDeque<Particula> particulasDisponiveis;
    ...
    private boolean resolucaoRapidaAplicada;
    ...
    public gameView(Context context, boolean modoUmJogador) {
        ...
        // O modo rapido usa uma superficie RGB 565. Ela precisa de metade da
        // largura de banda de uma superficie RGBA comum e combina com o cache
        // RGB 565 usado pelo fundo da arena.
        if (modoRapido) {
            holder.setFormat(PixelFormat.RGB_565);
        }

        paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintCache = new Paint(modoRapido
                ? Paint.DITHER_FLAG
                : Paint.FILTER_BITMAP_FLAG | Paint.DITHER_FLAG);
        random = new Random();
        bolas = new ArrayList<>();
        particulas = new ArrayList<>();
        particulasDisponiveis = new ArrayDeque<>(MAX_PARTICULAS);
    ...
    @Override
    public void run() {
        // Prioridade de exibicao reduz engasgos quando o Android executa
        // tarefas curtas em segundo plano durante a partida.
        Process.setThreadPriority(Process.THREAD_PRIORITY_DISPLAY);

        long tempoAnterior = System.nanoTime();
        // O jogo inteiro mira 60 FPS. O antigo alvo de 45 FPS do modo rapido
        // nao se encaixava de maneira uniforme em telas de 60/120 Hz e podia
        // parecer engasgado mesmo sem uso total do processador.
        long tempoFrameAlvo = TEMPO_FRAME_NS;
    ...
    public void surfaceCreated(SurfaceHolder surfaceHolder) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // Em telas de 90/120 Hz, pede uma cadencia de 60 FPS que o Android
            // consegue apresentar de maneira uniforme, sem quadros irregulares.
            surfaceHolder.getSurface().setFrameRate(
                    60f,
                    android.view.Surface.FRAME_RATE_COMPATIBILITY_FIXED_SOURCE
            );
        }

        if (modoRapido && !resolucaoRapidaAplicada) {
            int larguraView = getWidth();
            int alturaView = getHeight();

            if (larguraView > 0 && alturaView > 0) {
                float pixels = (float) larguraView * alturaView;
                float escalaPixels = (float) Math.sqrt(
                        MAX_PIXELS_MODO_RAPIDO / Math.max(1f, pixels)
                );
                float escalaLargura =
                        LARGURA_MAXIMA_MODO_RAPIDO / larguraView;
                float escala = Math.min(
                        1f,
                        Math.min(escalaPixels, escalaLargura)
                );

                if (escala < 0.98f) {
                    resolucaoRapidaAplicada = true;
                    surfaceHolder.setFixedSize(
                            Math.max(1, Math.round(larguraView * escala)),
                            Math.max(1, Math.round(alturaView * escala))
                    );
                    return;
                }
            }
    ...
    private void integrarMovimentoBolas(float dt) {
        ...
        passos = Math.max(
                1,
                Math.min(passos, modoRapido ? 4 : 12)
        );
    ...
    private void criarExplosao(
        ...
        while (particulas.size() + quantidade > limiteParticulas
                && !particulas.isEmpty()) {
            reciclarParticula(particulas.remove(0));
        }

        for (int i = 0; i < quantidade; i++) {
            Particula particula = particulasDisponiveis.pollFirst();
            if (particula == null) {
    ...
    private void reciclarParticula(Particula particula) {
        if (particulasDisponiveis.size() < MAX_PARTICULAS) {
            particulasDisponiveis.offerLast(particula);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (acao == MotionEvent.ACTION_DOWN
            ...
            float x = converterToqueX(event.getX(indice));
            float y = converterToqueY(event.getY(indice));
        ...
        if (acao == MotionEvent.ACTION_MOVE) {
            for (int i = 0;
                ...
                float x = converterToqueX(event.getX(i));
    ...
    /**
     * Quando o modo rapido reduz a resolucao interna, os toques continuam
     * chegando nas coordenadas fisicas da View. A conversao abaixo mantem as
     * raquetes exatamente sob o dedo.
     */
    private float converterToqueX(float x) {
        int larguraView = getWidth();
        return larguraView > 0
                ? x * larguraTela / larguraView
                : x;
    }

    private float converterToqueY(float y) {
        int alturaView = getHeight();
        return alturaView > 0
                ? y * alturaTela / alturaView
                : y;
    }
    ...
    private void desenhar() {
        ...
        try {
            // Em Android 8 ou superior, o HardwareCanvas manda circulos,
            // linhas, textos e bitmaps para a GPU. Em aparelhos antigos ou
            // drivers problemáticos, o jogo volta automaticamente ao Canvas
            // tradicional.
    ...
    private void atualizarCacheArena() {
        ...
        // A superficie do modo rapido ja foi reduzida. Manter o cache em 1:1
        // evita redimensionar um bitmap de fundo em todos os quadros.
        float escala = modoRapido ? 1f : 0.68f;
